...
function chartSkylineContinuous(days, t, box, { anim, speed, sky = "auto", now } = {}) {
  ...
  const encodingLegend = detail
    ? `<g class="f skyline-encoding" style="${delay(3, .08, speed)}" fill="${legendColor}" fill-opacity=".62"><title>Building height represents daily token activity</title><rect x="${legendX}" y="${legendBase - 3}" width="2.4" height="3" rx=".4"/><rect x="${legendX + 4}" y="${legendBase - 7}" width="2.4" height="7" rx=".4"/><rect x="${legendX + 8}" y="${legendBase - 11}" width="2.4" height="11" rx=".4"/><text x="${legendX + 14}" y="${legendBase - 1}" font-size="7" font-weight="600" letter-spacing=".65">DAILY TOKENS &#8594; SKYLINE HEIGHT</text></g>`
    : "";

...
The full-width Activity Skyline keeps its header compact and places a three-part legend directly below the city: `DAILY TOKENS → HEIGHT`, `18/30 ACTIVE`, and `7D STREAK`. Building height is the daily token total, `ACTIVE` is the number of token-active days in the displayed window, and the green route at the base marks the current trailing run of token-active days. If that run reaches the left edge of the selected window, the card uses `≥` because the actual streak may have begun earlier. The compact Skyline keeps the artwork clean and carries the same explanation in the SVG accessibility description.
